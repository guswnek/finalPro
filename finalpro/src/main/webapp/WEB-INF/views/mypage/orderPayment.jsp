...
<script>
	window.onload = function() {
		...
		document.getElementById("submitButton").value = formattedPrice + "원 결제하기";
	...
	<form>
					...
					<td>
						<c:forEach var="address" items="${addressList }" varStatus="status">
							<input type="radio" onclick="showAddressInfo()" name="addressName" 
								<c:if test="${status.index == 0}">checked="checked"</c:if> value="${address.addressName}">
								${address.addressName}
						</c:forEach>
		...
		<div class = "productInfo">
			...
			<table class = "table">
				...
				<c:forEach var = "recepit" items="${recepitList }" >
					<input type = "hidden" name = "noList" value = "${recepit.no }">
					<tr>
						<td align="center" width="7%" height = "95px">
							<a href = "${contextPath }/product/detailProduct.do?productNo=${recepit.productNo}">
							<img width="80px" height = "90px" src="${contextPath }/download.do?imageFileName=${recepit.imageFileName}&productNo=${recepit.productNo}&path=product" alt = "${recepit.name }">
							</a>
						</td>
						<td align="left" width ="15%"><b style = "font-size:13px; margin : 0 10px;">
							<a href = "${contextPath }/brand/brandList.do?brand=${recepit.brand}">[${recepit.brand }]</a>
							<a href = "${contextPath }/product/detailProduct.do?productNo=${recepit.productNo}">${recepit.name }</a></b><br>
						<span class = "option">옵션 : ${recepit.productSize }</span>
						</td>
						<td>${recepit.count }개</td>
						<td>무료</td>
						<td>${recepit.orderPrice }원</td>
					</tr>
				</c:forEach>
			</table>
